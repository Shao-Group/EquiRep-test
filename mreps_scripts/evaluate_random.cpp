...
string getMinLengthSeq(string true_seq, vector<string> predicted_seqs);
...
{
    ifstream predictedFile;
    ofstream outputFile;

    string predicted_fname = argv[1];
    ...
    int cas = 50;
    int edit_0 = 0, edit_1 = 0, edit_2 = 0, edit_3 = 0, edit_4 = 0, edit_g_4 = 0;
    vector<int> edits_list;

    for (int i = 0; i < cas; ++ i) 
    {
        string tmp, true_seq = "", predicted_seq = "";
        ...
        {
            predicted_seq = predicted_seqs[0];
        }

        if(true_seq.size() == 0 && predicted_seq.size() == 0)
        {
            outputFile << "predicted:" << predicted_seq <<endl;
            outputFile << "Edit: 0" << endl << endl;
            edit_0++;
            edits_list.push_back(0);
            continue;
        }
        ...
        {
            outputFile << "predicted:" << predicted_seq <<endl;
            outputFile << "Edit: " << predicted_seq.size() << endl << endl;
            if(predicted_seq.size() == 1) edit_1++;
            else if(predicted_seq.size() == 2) edit_2++;
            else if(predicted_seq.size() == 3) edit_3++;
            else if(predicted_seq.size() == 4) edit_4++;
            else if(predicted_seq.size() > 4) edit_g_4++;
            edits_list.push_back(predicted_seq.size());
            continue;
        }

        //get the best of the multiple predicted sequences
        predicted_seq = get_best_sequence(true_seq, predicted_seqs);
        outputFile << "predicted:" << predicted_seq <<endl;
    ...
    outputFile << edit_0 << "," << edit_1 << "," << edit_2 << "," << edit_3 << "," << edit_4 << "," << edit_g_4 << " (" << avg << ")" << endl;
}

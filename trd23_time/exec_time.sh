...
if [ $? -eq 0 ]; then
    ...
    do
        ...
        do
            ...
            time_file="$output_dir/timing_${output_file}.txt"
            echo "./$code $input_dir/$input_file $output_dir/$output_file"
            
            # Use 'time' command and redirect its output to a timing file
            /usr/bin/time -o "$time_file" ./$code "$input_dir/$input_file" "$output_dir/$output_file" 
            # &
